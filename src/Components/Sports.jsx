...
function Accessories() {
  ...
  return (
    <Row>
      ...
      <Col sm={9}>
        <div className='body'>
            <h2 className='heading'>Sportswear</h2>
            <Link to='/toproducts'><ArrowBackIcon className='previous'/></Link>
            <div className='cards'>
            {gymItem.map((gym)=>(<Link style={{textDecoration:'none', width: '30%', margin: '1%'}} key={gym.id} to={'/sportinfo/' + gym.id}><Gymcard gymData={gym}/></Link>))}
            </div>
        </div>
      </Col>
    </Row>
